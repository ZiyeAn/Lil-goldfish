let fox=[]
let colors=['#2b0f08','#5c2314','#7e4a18','#4d4a3e','#70591e','#2c291d','#282107']
...
function draw(){
  ...
  stroke(c5)
  strokeWeight(3)
  rect(windowHeight/2,windowHeight/2,0.98*windowHeight,0.98*windowHeight)
  strokeWeight(1,5)
  rect(windowHeight/2,windowHeight/2,0.96*windowHeight,0.96*windowHeight)
  pop()
  
  //rotation
  let sec = second();
  let secAngle = map(sec, 0, 60, 0, TWO_PI);
  currentAngle = lerp(currentAngle, secAngle, 0.1);
  let rotateAngle=map(mouseX, 0, windowWidth,0,PI)
  if (0<rotateAngle<PI){
      design(windowHeight/2,windowHeight/2,windowHeight/2,rotateAngle+currentAngle)
  }
  else{
    design(windowHeight/2,windowHeight/2,windowHeight/2,0)
  }
  ...
  imageMode(CENTER)
  if( 0 <= a <= 4){
    image(fox[a],windowHeight/2,windowHeight/2,600,600)
  }
  else{
    image(fox[0],windowHeight/2,windowHeight/2,600,600)
  }
}
